...
**Última atualização:** 2026-07-30 (depois do merge dos dois branches)
...
ainda**
**🔴 Antes de qualquer coisa:** nem a **party** nem o **PvP** foram testados com
dois clientes de verdade. Convite → aceite → XP dividida, e agredir → caveira →
revide, exigem duas janelas — nenhum dos dois lados conseguiu fazer sozinho
**Saúde do código:** **377 testes** passando (357 shared + 20 server) · typecheck limpo
...
1. **Ordem das camadas de defesa** (`31.56`) — ✅ **nada a decidir.** O aparente
   conflito entre o diagrama e o texto do cap. 31 não existe: a frase "a redução
   % age sobre o que sobrou" fala das reduções de buff/skill/carta, não do
   escudo — e essas já são a última camada. Fica a ordem do diagrama.
2. **Duração de Congelamento × Petrificação** (`DD-CC-012`) — ✅ **confirmada**
   pelo dono a revisão posterior: Congelamento 10 s, Petrificação 6 s.
3. **Método anti-CC-chain** (`DD-CC-013/014`) — ✅ **confirmado deixar de fora**
   enquanto o doc não define o método. Ponto de plugar marcado em
   `applyCondition`.
...
### ✅ Etapa 9 — Party, shared XP e distribuição de loot 👥 *(completa em 2026-07-30)*

> 🔴 **Esta etapa foi fechada por um MERGE, e a história vale ser lembrada:** os
> dois irmãos escreveram metades complementares em paralelo, sem saber. Um fez a
> **formação** (convite com prazo de 30 s revalidado no aceite, expulsar, passar
> liderança, `PARTY_MAX`), o outro fez a **distribuição** (shared XP, três regras
> de loot com votação, loot de chefe por contribuição). Nenhuma das duas metades
> fechava a etapa sozinha.
>
> Detalhe de cada decisão em [`HISTORICO.md`](./HISTORICO.md); as regras puras
> vivem em `shared/src/party.ts`.
>
> ⚠️ **Ainda não foi testada com dois clientes de verdade.**

Além dos seis pontos abaixo, entrou o que a formação trouxe: `partyId` chega ao
`canHarm` → **friendly fire desligado entre companheiros**, que era a única regra
de party que o jogo tinha e não usava.

⚠️ **`PARTY_MAX = 5` é REFERÊNCIA, não canônico.** `DD-PARTY-001..026` não fixam
tamanho de grupo em lugar nenhum — o 5 sai do exemplo de distribuição de XP do
cap. 35. Nenhum teste trava o valor.

**Os seis pontos, todos implementados:**
